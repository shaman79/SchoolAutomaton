...
def build_section_user(intent: StructuredIntent, *, kind: str, title: str, objective: str | None) -> str:
    ...
    return (
        f"Fill the '{kind}' section of the lesson below. Produce a GenSection.\n\n"
        f"{_intent_context(intent)}\n\n"
        f"section_kind: {kind}\n"
        ...
        f"{language_directive(intent.language)}\n\n"
        "Requirements:\n"
        "- body_markdown: clear, dual-coded prose at or below the target reading level; keep sentences "
        "within the word cap and define new terms inline.\n"
        "- For pretest/practice/interleaved_review/misconception_check sections, include items "
        "(GenItem): item_type, concept_slug, bloom_tier (1-6), difficulty, item_difficulty (1-5), "
        "stem_markdown, typed payload, distractors (one per wrong MCQ option: `text` repeats that option's "
        "text exactly, plus a misconception string naming the wrong idea it reflects, or null), "
        "hint_ladder (1-3), worked_solution_steps, explanation. MCQ has one correct option unless "
        "multiple-select; list options in no fixed order (never correct-first), and give "
        "multiple-select at least as many distractors as correct options.\n"
        "- Only pretest/practice/interleaved_review/misconception_check sections have items; for every "
        "other section set items to [].\n"
        "- visual_requests: 0-2 GenVisualSpec entries where a visual aids understanding, each with "
        "alt_text; set section_ordinal to 0 (it refers to this section).\n"
        "- Never reveal the answer in a stem."
    )
...
def build_quiz_user(
    ...
    *,
    ...
    return (
        "Generate an interactive quiz for the validated intent below. Produce a GenQuiz. Write the "
        f"`title` and every question's text in {lang} (the content language) — never leave the title "
        "in English.\n\n"
        f"{_intent_context(intent)}\n\n"
        ...
        "Requirements:\n"
        "- Choose the number of questions to fit the topic's complexity and breadth — about 5 for a "
        "narrow/simple topic, up to 12 for a rich one. Vary it to suit the subject; do NOT default to a "
        "fixed count.\n"
        f"- Span the Bloom tiers suited to the grade band, placed so a learner at mastery {mastery:.2f} "
        f"succeeds about {target_success:.2f} of the time.\n"
        "- Mix item types. MCQ has exactly one correct option unless multiple-select; give plausible "
        "distractors (one per wrong option: `text` repeats that option's text exactly, plus a "
        "misconception string naming the wrong idea it reflects, or null). List options in no fixed order — do "
        "NOT put the correct option(s) first; for multiple-select include at least as many distractors "
        "as correct options.\n"
        "- Each item: item_type, concept_slug, bloom_tier (1-6), difficulty, item_difficulty (1-5), "
        "stem_markdown, typed payload, hint_ladder (1-3), worked_solution_steps, explanation. Target "
        "the listed known misconceptions where relevant.\n"
        "- Never reveal the answer in a stem."
    )
